...
//Teuchos::RCP<array <Teuchos::RCP<array <Teuchos::RCP <Epetra_Vector> > > > > Kx_table[sz][sz];
//Teuchos::RCP< Teuchos::RCP< Epetra_Vector> > Kx_table;
//Teuchos::Array< Teuchos::Array< Teuchos::RCP<Epetra_Vector> > > Kx_table;
...
for (int i=0;i<sz;i++) {  
  ...
  for (int j=0;j<expn;j++) {
    Kx_table[i][j] = Teuchos::rcp(new Epetra_Vector(*(app->getMap())));
  }
}

//(sg_dx_vec_all[0])->PutScalar(1.0);

//(*sg_J_poly)[0].Apply(*(sg_dx_vec_all[0]),*(Kx_table[0][0]));

//df->Update(1.0, *(Kx_table[0][0]), 0.0);

//std::cout <<"Kx_table test " << *(Kx_table[0][0]) << std::endl;
...
while ((norm_df/norm_f)>1e-12) {
  ...
  //      for (int i=0;i<num_KL+1;i++) 
  //        for (int l=0; l<sz; l++)
  //          (*sg_J_poly)[i].Apply(*(sg_dx_vec_all[l]),*(Kx_table[l][i])); 
...
//          if (i !=0 && i<num_KL+1) {
          if (i !=0 ) {
//           (*sg_J_poly)[i].Apply(*(sg_dx_vec_all[j]),*(Kx_table[j][i]));
           df->Update(-1.0*c,*(Kx_table[j][i]),1.0);          
          }
          if (i==0 && j==k) {
            c0kk = c;
...
//      std::cout << "df " << *df << std::endl;
    ...
    for (int i=1;i<num_KL+1;i++) {
     //for (int l=0; l<sz; l++)
        //int nj = Cijk->num_j(i);
        //const Teuchos::Array<int>& j_indices = Cijk->Jindices(i);
      //  std::cout << "j ind i= " << i <<" ->  "<< nj <<"-->> " <<j_indices << std::endl;
       for (int jj=0;jj<nj;jj++) {
          int j = j_indices[jj];
          if (j==k) {
//          const Teuchos::Array<int>& i_indices = Cijk->Iindices(i,jj);
//          int ni = i_indices.size();
//          for (int ii=0;ii<ni;ii++) {
      //      int i = i_indices[ii];
//      (*sg_J_poly)[i].Apply(*(sg_dx_vec_all[k]),*(Kx_table[k][i]));        
      (*sg_J_poly)[i].Apply(*(sg_dx_vec_all[j]),*(Kx_table[j][i]));        
//      std::cout << "*(Kx_table[j][i])" << k << *(Kx_table[k][i]) << std::endl;
//	    std::cout << "dx(0):" << std::endl
//	          << *(sg_dx_vec_all[k]) << std::endl;
          }
          }
        }
     // }
    //  std::cout << "*(Kx_table[j][i])" << k << *(Kx_table[k][1]) << std::endl;
//      for (int iind=1;iind<num_KL+1;iind++) 
//	  (*sg_J_poly)[iind].Apply(*(sg_dx_vec_all[k]),*(Kx_table[k][iind]));        
...
//(*sg_J_poly)[0].Apply(*(sg_dx_vec_all[0]),*(Kx_table[0][0]));
// std::cout << "*(Kx_table[j][i]) before" << *(Kx_table[0][0]) << std::endl;
  //   sg_dx_vec_all[0]->Update(1.0, *sg_dx_vec_all[0], 1.0);

//(*sg_J_poly)[0].Apply(*(sg_dx_vec_all[0]),*(Kx_table[0][0]));
// std::cout << "*(Kx_table[j][i]) after" << *(Kx_table[0][0]) << std::endl;
