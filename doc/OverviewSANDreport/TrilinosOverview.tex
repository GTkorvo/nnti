...
substantially reduced because our common infrastructure provides an excellent starting point.  Furthermore, many applications are standardizing on the 
...
\item[Petra]
A Greek term meaning ``foundation.''  Trilinos has three Petra libraries: 
Epetra, Tpetra and Jpetra (discussed in 
Section~\ref{subsect:PetraObjectModel}) that provide basic classes for 
constructing and manipulating matrix, graph and vector objects.
...
\item[TSF] Collection of abstract interfaces for accessing solver
functionality without explicit dependence on concrete classes.  TSF is
spread across three packages:  TSFUtils, TSFCore and TSFExtended.
\item[NOX] A collection of nonlinear solvers implemented in a
self-contained package.  NOX contains basic Newton, line search and
tensor-based methods for solving nonlinear equations.  It is designed
to be easily integrated into an existing application.  It can work
with Epetra, AztecOO, PETSc and LAPACK as the underlying linear
algebra services packages.
\item[ML] A collection of geometric and algebraic multigrid methods and
domain decomposition methods.  The algebraic methods are based primarily 
on the ideas of smoothed aggregation.  The methods in ML can be used either as linear solvers or as preconditioners to an outer iterative method.
ML has no explicit dependencies on any other Trilinos packages and can be
built as a standalone package.
\item[Komplex] Complex linear equation solver using equivalent real 
formulations, built on top of Aztec.
\item[Ifpack] Object-oriented algebraic preconditioner, compatible with 
Epetra and AztecOO.
...
web-based interface package called Bonsai~\cite{Bonsai}.  Features 
