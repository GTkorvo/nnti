...
and the {\it this} is being constructed and used.
...
void TSFComposedOperator::apply(const TSFVector& in, TSFVector& out) const {
...
aggregations can be found in the TSF User Guide~\cite{TsfUserGuide}.
...
AztecOO is an object-oriented follow-on to Aztec~\cite{Aztec2.1}.  As such, it provides
all of the same capabilities as Aztec, but provides more elegant interface and some extended
functionality.  Aztec specifically solves a linear system $AX=B$ where $A$ is a linear 
operator, $X$ is a multivector containing one or more initial guess on entry and the
corresponding solutions on exit, and the corresponding right-hand-sides in $B$.
...
the  Epetra\_Operator or Epetra\_RowMatrix interfaces, something that is not too difficult
to do for most linear solver libraries.
...
classes derive from Epetra\_RowMatrix)
...
a form compatible with the original complex-valued problem.

\subsection{NOX: Nonlinear Solver Package}

NOX provides access to a suite of nonlinear solver methods.  It can be easily integrated
...
and can be easily extended as new nonlinear methods are developed.  Presently, it contains
the following methods:
\begin{enumerate}
\item blah, blah, blah
\end{enumerate}

NOX uses pure virtual classes to work with all matrix, operator and vector objects.  Therefore,
it can be used with any external software that can implement these interfaces.  Epetra and
PETSc implementations of the pure virtual classes are already implemented.
...
independent solver developers, making integration of a package attractive to developers.
The ultimate result of developing the Trilinos framework is that it minimizes redundant
effort on the part of solver developers by providing an existing set of tools  
