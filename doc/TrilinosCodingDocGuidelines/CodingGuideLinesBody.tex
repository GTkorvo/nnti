...
the guideline will be considered to violate and is {}\tit{strongly
recommended} will be considered not be directly interoperable with
compliant software.
...
use of {}\ttt{const} is considered primarily a safety issue.
...
Every guideline is given a recommendation strength and at least on
...
\item The header file \ttt{SomeClass.hpp} is designed to be included
  ...
  \ttt{SomeClass\_def.hpp} is hidden from general clients and changes
  in it do not require recompilation of client code.  The file
  \ttt{SomeClass.hpp} can (and should) be automatically configured by
