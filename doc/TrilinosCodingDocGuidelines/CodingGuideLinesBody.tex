...
\item\GCGStdStream: Sending output directly to \texttt{std\-::cout} or
  ...
  \item\GCGTeuchosVerboseObject: Clients may either set the output
    stream and the verbosity level through a parameter list (see the
    \texttt{Teuchos\-::ParameterListAcceptor} interface described
    above), or directly in code.  If no output stream is set, then
