...
Here we derive direct and adjoint sensitivity methods for general ODEs and
DAEs.  The general DAE model is
%
\begin{eqnarray}
f\left( \dot{x}(t), x(t), p, v(t), t \right) & = & 0,
\; t \in \left[ t_0, t_f \right] \label{rythmos:apdx:eqn:sens:f} \\
x(0) & = & x_0(p), \label{rythmos:apdx:eqn:sens:f:ic} \\
\dot{x}(0) & = & \dot{x}_0(p), \label{rythmos:apdx:eqn:sens:f:ic-dot}
\end{eqnarray}
%
...
\end{equation}
%
where $x: t {}\rightarrow x(t){}\:\in\:\mathcal{X}$ for $t\in[t_0,t_f]$,$\dot{x}: t {}\rightarrow \dot{x}(t) = d(x)/d(t)\:\in\:\mathcal{X}$ for
$t\in[t_0,t_f]$, $p{}\:\in\:\mathcal{P}$, $v: t {}\rightarrow
v(t){}\:\in\:\mathcal{V}$ for $t\in[t_0,t_f]$, $f(\dot{x}, x, p, v, t)
:{}\mathcal{X}^2{}\times{}\mathcal{P}{}\times{}\mathcal{V}{}\times{}\RE
{}\rightarrow{}\mathcal{C}$,
$x_0(p){}\:\in\:\mathcal{P}{}\rightarrow{}\mathcal{X}$, $g(\dot{x},x,p,v,t)
:{}\: {}\mathcal{X}^2{}\times{}\mathcal{P}{}\times{}\mathcal{V}{}\times{}\RE
{}\rightarrow{}\mathcal{G}$, $h(\dot{x},x,p) :{}\:
{}\mathcal{X}^2{}\times{}\mathcal{P} {}\rightarrow{}\mathcal{G}$,
$\mathcal{G}{}\:\subseteq\:\RE^{n_g}$, $\mathcal{X}{}\:\subseteq\:\RE^{n_x}$,
$\mathcal{P}{}\:\subseteq\:\RE^{n_p}$, $\mathcal{V}{}\:\subseteq\:\RE^{n_v}$,
and $\mathcal{C}{}\:\subseteq\:\RE^{n_x}$.

{}\textit{Remarks on notation}: We use the commonly accepted convention where
the unadorned indentifier $x$ for a function such as $t {}\rightarrow x(t)$,
...
omitted and instead $x$ is used and it should be clear from the context (i.e.\
...
(\ref{rythmos:apdx:eqn:sens:f})--(\ref{rythmos:apdx:eqn:sens:f:ic-dot}) at points
...
\section{Derivation of direct sensitivities}
...
\>  $\jac{x}{p} \in \mathcal{X}|\mathcal{P}$ is the direct sensitivity of the state defined on $t\in[t_0,t_f]$, \\
\>  $\jac{\dot{x}}{p} \in \mathcal{X}|\mathcal{P}$ is the direct sensitivity of the state time derivative defined on $t\in[t_0,t_f]$, and \\
\>  $\jac{\hat{d}}{p} \in \mathcal{G}|\mathcal{P}$ is defined independent of time.
\end{tabbing}

The direct sensitivity $\jac{x}{p}$ and $\jac{\dot{x}}{p}$ in
...
differentiating
(\ref{rythmos:apdx:eqn:sens:f})--(\ref{rythmos:apdx:eqn:sens:f:ic-dot}) with
respect to $p$ to give
%
\begin{eqnarray}
%
\Jac{f}{\dot{x}} \left(\Jac{\dot{x}}{p} \right) + \Jac{f}{x} \left(\Jac{x}{p}\right)
+ \Jac{f}{p} & = & 0, \; t \in \left[ t_0, t_f \right], \label{rythmos:apdx:eqn:sens:direct-f} \\
\Jac{x(t_0)}{p} & = & \Jac{x_0}{p}, \label{rythmos:apdx:eqn:sens:direct-f:ic} \\
\Jac{\dot{x}(t_0)}{p} & = & \Jac{\dot{x}_0}{p}, \label{rythmos:apdx:eqn:sens:direct-f:ic-dot}
\end{eqnarray}
\begin{tabbing}
\hspace{4ex}where\hspace{1ex}\= \\
\>	$\jac{f}{\dot{x}} \in \mathcal{C}|\mathcal{X}$ is defined on $t\in[t_0,t_f]$, \\
\>	$\jac{f}{x} \in \mathcal{C}|\mathcal{X}$ is defined on $t\in[t_0,t_f]$, \\
\>	$\jac{f}{p} \in \mathcal{C}|\mathcal{P}$ is defined on $t\in[t_0,t_f]$, \\
...
(\ref{rythmos:apdx:eqn:sens:f})--(\ref{rythmos:apdx:eqn:sens:f:ic-dot}) and
(\ref{rythmos:apdx:eqn:sens:direct-f})--(\ref{rythmos:apdx:eqn:sens:direct-f:ic-dot}).
Note that (\ref{rythmos:apdx:eqn:sens:f})--(\ref{rythmos:apdx:eqn:sens:f:ic-dot})
and
(\ref{rythmos:apdx:eqn:sens:direct-f})--(\ref{rythmos:apdx:eqn:sens:direct-f:ic-dot})
are a staggered set of DAE equations in that
(\ref{rythmos:apdx:eqn:sens:f})--(\ref{rythmos:apdx:eqn:sens:f:ic-dot}) are solved
first followed by
(\ref{rythmos:apdx:eqn:sens:direct-f})--(\ref{rythmos:apdx:eqn:sens:direct-f:ic-dot}).

\section{Derivation of adjoint sensitivities}

{}\textbf{2007/06/13: rabartl: ToDo:} Update the below derivations for adding
the dependence of $\dot{x}$ in the response functions.
...
$\jac{\hat{d}}{p}$ in (\ref{rythmos:apdx:eqn:sens:d_d_hat_d_p}) and the direct
sensitivity equations for $\jac{x}{p}$ in
(\ref{rythmos:apdx:eqn:sens:direct-f})--(\ref{rythmos:apdx:eqn:sens:direct-f:ic-dot})
...
equation (\ref{rythmos:apdx:eqn:sens:direct-f}) which is
%
\begin{equation}
...
+ \Jac{f}{p}
\right) dt = 0,
\label{rythmos:apdx:eqn:sens:weak-direct-f}
\end{equation}
%
...
the weak form of (\ref{rythmos:apdx:eqn:sens:weak-direct-f}) is every bit as
valid and is indeed more general than the strong form in
(\ref{rythmos:apdx:eqn:sens:direct-f}) and we lose nothing by considering the
...
\end{equation}
%
into (\ref{rythmos:apdx:eqn:sens:weak-direct-f}) and rearrange which yields
%
\begin{equation}
...
+ \left. \left( \Lambda^T \Jac{f}{\dot{x}} \Jac{x}{p} \right) \right|_{t_0}^{t_f}
= 0.
\label{rythmos:apdx:eqn:sens:weak-direct-f-2}
\end{equation}
%
...
(\ref{rythmos:apdx:eqn:sens:weak-direct-f-2}) and rearranging which yields
%
\begin{eqnarray}
\int_{t_0}^{t_f} \left( \Jac{g}{x} \Jac{x}{p} \right) dt
& = &  \int_{t_0}^{t_f} \left( \Lambda^T \Jac{f}{p} \right) dt
+ \left. \left( \Lambda^T \Jac{f}{\dot{x}} \Jac{x}{p} \right) \right|^{t_f}_{t_0}.
\label{rythmos:apdx:eqn:sens:weak-direct-f-3}
\end{eqnarray}

Finally, substituting (\ref{rythmos:apdx:eqn:sens:weak-direct-f-3}) into
...
We start the proof by substituting (\ref{rythmos:apdx:eqn:sens:direct-f}) into
...
a solution for $f(\dot{x},x,p,v,t)=0$) with respect to $v$ along the direction
$\delta v$ which is
%
\begin{equation}
%
...
\end{equation}
%
into (\ref{rythmos:apdx:eqn:sens:weak-direct-f-v}) and rearranging yeilds
%
\begin{equation}
...
+ \left. \left( \Lambda^T \Jac{f}{\dot{x}} \Jac{x}{v} \delta v \right) \right|_{t_0}^{t_f}
= 0.
\label{rythmos:apdx:eqn:sens:weak-direct-f-v-2}
\end{equation}
%
Substituting the choice for the adjoint equation
(\ref{rythmos:apdx:eqn:sens:adj-trans}) into
(\ref{rythmos:apdx:eqn:sens:weak-direct-f-v-2}) and rearranging yeilds
%
\begin{eqnarray}
\int_{t_0}^{t_f} \left( \Jac{g}{x} \Jac{x}{v} \delta v \right) dt
& = &  \int_{t_0}^{t_f} \left( \Lambda^T \Jac{f}{v} \delta v \right) dt
+ \left. \left( \Lambda^T \Jac{f}{\dot{x}} \Jac{x}{v} \delta v \right) \right|^{t_f}_{t_0}.
\label{rythmos:apdx:eqn:sens:weak-direct-f-v-3}
\end{eqnarray}
%
Substituting (\ref{rythmos:apdx:eqn:sens:weak-direct-f-v-3}) into
