...
thought of as the linear operator $x\in\mathcal{X}|\RE$ an it's adjoint can be
applied to another vector $v\in\mathcal{X}$ as $x^T v {}\in\RE$ which is
...
$Y\in\mathcal{Y}|\mathcal{X}$ such as
%
\[
Y = A X
\]
...
other linear operators and as linear operators) greatly simplifies the
notation of various formulations.
