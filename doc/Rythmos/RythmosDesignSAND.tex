...
spaces (i.e.\ $<A u,v>_{\mathcal{F}} = <A^H v,u>_{\mathcal{X}}$ where
...
=
{\bmat{cc}
\alpha \Jac{c_1}{\dot{y}_1} + \beta \Jac{c_1}{x_1}
& 0 \\
\beta \Jac{c_2}{x_1}
& \alpha \Jac{c_2}{\dot{y}_2} + \beta \Jac{c_2}{x_2} \\
\emat}.
\end{equation}
%
...
(\jac{c_2}{x_1})$ which is not required by any of the individual DAE solvers
...
method, only the action of $(\jac{c_2}{x_1}) V$ are to arbitrary multivectors
$V\in\mathcal{C}_2|\RE^m$ are needed which can be approximated using
...
involving $(\jac{c_2}{x_1})$ which are not required when solving the DAEs
separately.
%
\end{itemize}
%
...
\>	$y$ is shorthand for the infinite dimensional function $y(t)$ in $t\in[t_0,t]$, and \\
\>	$v$ is shorthand for the infinite dimensional function $v(t)$ in $t\in[t_0,t]$.
\end{tabbing}

Note that the domain space for the overall response
$d(y,p,v$ is infinite since $y$ and
$v$ are infinite in the continuous formulation.

The state DAE in
(\ref{rythmos:eqn:sens:c})--(\ref{rythmos:eqn:sens:c:ic}) may be used
to define the implicit state function $y(p,v,t)$.  The full
implicit state solution for $y(t)$ in $t\in[t_0,t_f]$ is signified as
$y(p,v,t_0:t_f)$.

The implicit state solution $y(p,v,t)\in\mathcal{Y}$ substituted into
...
\> $\dot{S} = \left(\frac{d}{dt} \Jac{y(t)}{p}\right)
...
%
\begin{eqnarray}
- \frac{d}{dt}\left( \Jac{c}{\dot{y}}^H \Lambda \right)
+  \Jac{c}{y}^H \Lambda + \Jac{g}{y}^H
& = & 0, \; t \in \left[ t_0, t_f \right],
\label{rythmos:app:eqn:sens:adj} \\
\left.\left( \Jac{c}{\dot{y}}^H \Lambda \right)\right|_{t=t_f}
& = & - \left. \Jac{h}{y}^H \right|_{y=y(t_f)},
...
{\bmat{c}
%
- \Jac{c}{\dot{y}}^H \dot{\Lambda}_{(:,1)}
+  \Jac{c}{y}^H \Lambda_{(:,1)} + \Jac{g_{(1)}}{y}^H
%
\\ \vdots \\
%
- \Jac{c}{\dot{y}}^H \dot{\Lambda}_{(:,n_g)}
+  \Jac{c}{y}^H \Lambda_{(:,n_g)} + \Jac{g_{(n_g)}}{y}^H
%
\emat}
...
= 
{\bmat{ccc}
\alpha \Jac{c}{\dot{y}}^H + \beta \Jac{c}{y}^H \\
& \ddots \\
& & \alpha \Jac{c}{\dot{y}}^H + \beta \Jac{c}{y}^H
\emat}
...
+ \Jac{c}{y}^H \Lambda_{(:,1)} + \Jac{g_{(1)}}{y}^H
\\
\bar{\Lambda}_{(:,1)} + \Jac{c}{\dot{y}}^H \Lambda_{(:,1)}
\emat} \\
%
\vdots \\
%
...
+ \Jac{c}{y}^H \Lambda_{(:,n_g)} + \Jac{g_{(n_g)}}{y}^H
\\
\bar{\Lambda}_{(:,n_g)} + \Jac{c}{\dot{y}}^H \Lambda_{(:,n_g)}
\emat} \\
%
\emat}
...
%
{\bmat{cc}
\alpha I & \beta \Jac{c}{y}^H \\
\beta I &  \beta \Jac{c}{\dot{y}}^H
\emat} \\
%
& \ddots \\
%
...
\begin{equation}
{\bmat{cc}
 & \bar{\alpha} \Jac{c}{\dot{y}}^H + \bar{\beta} \Jac{c}{y}^H \\
\beta I &  \beta \Jac{c}{\dot{y}}^H
\emat}
{\bmat{c} v_1 \\ v_2 \emat}
=
...
%
\[
v_2 = \left( \bar{\alpha} \Jac{c}{\dot{y}}^H + \bar{\beta} \Jac{c}{y}^H \right)^{-1}
...
%
\[
v_1 = \frac{1}{\beta} \left( b_2 - \beta \Jac{c}{\dot{y}}^H v_2 \right).
\]
%
The matrix
%
...
\int_{t_0}^{t_f} \left(
    ...
    + \Jac{c}{p}^H \Lambda
  \right) dt
  + \left. \Jac{h}{p}^H \right|_{t=t_f}
  - \left. \left( \Jac{y_0}{p}^H \Jac{c}{\dot{y}}^H \Lambda \right) \right|_{t=t_0}
...
separately.  Of course the operator returned for $M = \jac{c}{dot{y}}$ for the
case of $(\alpha,\beta)=(1,0)$ is not guaranteed to be nonsingular.  Only
operators $M$ for reasonable finite nonzero values of $\alpha$ and $\beta$ are
...
\[
(\{u_l\},U_l) {}\rightarrow \Jac{y_0}{u_l}^H V \in \mathcal{U}_l|\RE^m
\; \mbox{for} \; l = 1 {}\ldots N_u,
\]
or alternatively evaluate the sensitivity themselves as a multi-vector
\[
...
\]
%
{}\item Application of adjoint operator $M^H = ({}\alpha (\jac{c}{\dot{y}}) +
{}\beta (\jac{c}{y}))^H$: Given a pre-formed object for $M = {}\alpha
...
requirement.
%
{}\item Solve with adjoint operator $M^H = ({}\alpha (\jac{c}{\dot{y}}) +
{}\beta (\jac{c}{y}))^H$: Given a pre-formed object for $M = {}\alpha
...
the action of
\[
(\dot{y},y,\{u_l\},t,V) {}\rightarrow \Jac{c}{u_l}^H V \in \mathcal{U}_l|\RE^m,
\; \mbox{for} \; l = 1 {}\ldots N_u,
\]
or alternatively compute the sensitivities themselves as a multi-vector
\[
...
\[
(y,\{u_l\},U_l) {}\rightarrow \Jac{g_j}{u_l}^H V \in \mathcal{U}_l|\RE^m,
\; \mbox{for} \; l = 1 {}\ldots N_u, j = 1 \ldots N_g
\]
or alternatively compute the sensitivities themselves as a multi-vector
\[
...
the unadorned indentifier $y$ of a function such as $t {}\rightarrow y(t)$ for
$t\in[t_0,t_f]$ is used to represent the function of the the entire domain
$t\in[t_0,t_f]$.  When appropriate, the notation $y(t)$ will be used to
represent a particualar evaluation of the function at some time $t$ in the
domain $[t_0,t_f]$.  However, in some situations, the argument $t$ of $y(t)$
will be omitted and instead $y$ is used and it should be clear from the
context (i.e.\ inside of an integral) that really a single value of the
function is being represented.  Cases of possible ambiguity will be addressed.

The implicit state solution for the DAEs in
(\ref{rythmos:apdx:eqn:sens:c})--(\ref{rythmos:apdx:eqn:sens:c:ic}) at time
...
\>	$\jac{y}{p} \in \mathcal{Y}|\mathcal{P}$ is the direct sensitivity of the state
    at $t\in[t_0,t_f]$, and \\
\>	$\jac{\hat{d}}{p} \in \mathcal{G}|\mathcal{P}$ is defined independent of time.
\end{tabbing}

The direct sensitivity $\jac{y}{p}$ is computed by solving a set of $n_p$
independent direct sensitivity equations which are obtained by differentiating
(\ref{rythmos:apdx:eqn:sens:c})--(\ref{rythmos:apdx:eqn:sens:c:ic}) with
respect to $p$ to give
%
\begin{eqnarray}
%
\Jac{c}{\dot{y}} \left(\frac{d}{dt} \Jac{y(t)}{p} \right) + \Jac{c}{y} \left(\Jac{y(t)}{p}\right)
...
{}\in\mathcal{Y}|\mathcal{P}$ forward in time from $t=t_0$.  The integral in
(\ref{rythmos:apdx:eqn:sens:d_d_hat_d_p}) can be evaluated along with the
integration of
...
expressions that are based on straightforward principles that have a simple
...
\end{equation}
%
where at this point $\Lambda(t)\in\mathcal{C}|\mathcal{G}$, for
$t\in[t_0,t_f]$, is any appropriate weighting function\footnote{The admissible
class of wieghting functions $\Lambda(t)\in\mathcal{C}|\mathcal{G}$ are those
functions that are sufficiently smooth such that the integrals of the weak
form (and the modified weak form after integration by parts) are finite and
well defined.  Other requirements may also need to be satisfied in some cases
[???].}.  Later, $\Lambda$ will be chosen to be the adjoint variables but for
now $\Lambda$ is nothing more than an arbitrary weighting function for the
purpose of stating the weak form.  The solution to the weak form of
(\ref{rythmos:apdx:eqn:sens:weak-direct-c}) is every bit as valid and is
indeed more general than the strong form in
...
%
\begin{equation}
\int_{t_0}^{t_f} \left( \Lambda^H \Jac{c}{p} \right) dt
+ \int_{t_0}^{t_f} \left[
...
\begin{equation}
- \frac{d}{dt}\left( \Lambda^H \Jac{c}{\dot{y}} \right)
+  \Lambda^H \Jac{c}{y} = - \Jac{g}{y}, \; t \in \left[ t_0, t_f \right].
...
determine $\Lambda$ as no boundary conditions have yet be specified.  It will
...
& = &  \int_{t_0}^{t_f} \left( \Lambda^H \Jac{c}{p} \right) dt
+ \left. \left( \Lambda^H \Jac{c}{\dot{y}} \Jac{y}{p} \right) \right|^{t_f}_{t_0}.
...
\begin{eqnarray}
\Jac{\hat{d}}{p} 
& = & \int_{t_0}^{t_f} \left( \Lambda^H \Jac{c}{p} + \Jac{g}{p} \right) dt
...
in time from $t_f$ to $t_0$ [???].  However, as described in the following
theorem, we are free to pick almost any final condition for $\Lambda(t_f)$
that is consistent with the adjoint equation at $t_f$ and then evaluate the
terms in (\ref{rythmos:apdx:eqn:sens:d_d_hat_d_p-3}) involving the resulting
adjoint solution.
...
\nonumber \\
& = &
\int_{t_0}^{t_f} \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{p} dt
+ \left.\left[ \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{\dot{y}} \Jac{y}{p} \right]\right|_{t_0}^{t_f}
= 0
...
\begin{equation}
D_2 - D_1 = 
\int_{t_0}^{t_f} \left( \Lambda_2 - \Lambda_1 \right)^H \left[
...
%
\[
- \int_{t_0}^{t_f} \left[ \left( \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{\dot{y}} \right) \frac{d}{dt}\left( \Jac{y}{p} \right) \right] dt
= - \left. \left( \left( \Lambda_2 - \Lambda_1 \right)^H  \Jac{c}{\dot{y}} \Jac{y}{p} \right) \right|_{t_0}^{t_f}
+ \int_{t_0}^{t_f} \left[ \frac{d}{dt}\left( \left( \Lambda_2 - \Lambda_1 \right)^H  \Jac{c}{\dot{y}} \right) \Jac{y}{p} \right] dt
\]
%
...
\begin{equation}
D_2 - D_1 = 
\int_{t_0}^{t_f} \left[ \frac{d}{dt} \left( \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{\dot{y}} \right)
- \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{y} \right] \left(\Jac{y}{p}\right) dt.
...
%
\begin{equation}
\frac{d}{dt} \left( \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{\dot{y}} \right)
- \left( \Lambda_2 - \Lambda_1 \right)^H \Jac{c}{y} = 0, \; t \in [t_0,t_f].
...
derivative $\jac{\hat{d}}{p}$.

While the above theorem establishes that a wide variety of final conditions
for $\Lambda(t_f)$ can be chosen to close the adjoint equation, rather than
...
\begin{eqnarray}
\frac{d}{dt}\left( \bar{\Lambda} \right)
+  \Jac{c}{y}^H \Lambda + \Jac{g}{y}^H \Lambda
& = & 0, \; t \in \left[ t_0, t_f \right],
