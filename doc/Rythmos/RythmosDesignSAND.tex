...
\subsection{Weaker requirements (wants)}
...
There are a few points to be made about checkpointing and interpolation:
\begin{itemize}
%
{}\item An time stepper (or a series of staggered time steppers) restarted
form a checkpoint should produce the exact same (binary) output as if the
...
\subsection{Derivation of general direct sensitivity equations}
...
Given the implicit state function and ignoring the presence of $v(t)$, a
reduced set of auxiliary response functions are defined as
%
\begin{equation}
\hat{d}(p)
= \int_{t_0}^{t_f} \hat{g}(p,t) dt + \hat{h}(p),
\label{rythmos:apdx:eqn:sens:d_hat}
\end{equation}
\begin{tabbing}
\hspace{4ex}where\hspace{1ex}\= \\
\>	$\hat{g}(p,t) = g(y(p,t),p,t)) : \:
		\mathcal{P} \times \RE \rightarrow \mathcal{G}$ is defined on $t\in[t_0,t_f]$, and \\
\>	$\hat{h}(p) = h(y(p,t_f),p)) : \:
...
\subsection{Derivation of general adjoint sensitivity equations}
...
respect to $p$ and $v(t)$ for $t\in[t_0,t_f]$.  The primary devise that that
we use here is the Lagrangian which is a special function which is the
weighted composition of the response function and the DAE constraints.  This
derivation follows in a similary manner from the one in [???].  The Lagrangian
is
%
\begin{eqnarray}
...
(\ref{rythmos:apdx:eqn:sens:adj-trans}).  As stated in [???] for index-0 and
...
\end{equation}
%
which drops out the last term in
(\ref{rythmos:apdx:eqn:sens:d_L_hat_d_p_2}) and, together with
(\ref{rythmos:apdx:eqn:sens:adj-trans}), leaves
%
\begin{equation}
\frac{\partial \hat{d}}{\partial p} =
\int_{t_0}^{t_f} \left(
...
We now restate the adjoint and reduced sensitivities in
(\ref{rythmos:apdx:eqn:sens:adj-trans})--(\ref{rythmos:apdx:eqn:sens:adj-trans:fc})
and (\ref{rythmos:apdx:eqn:sens:d_d_hat_d_p_final}) in standard
columnwise form for $\Lambda$ which yields the adjoint DAE equations
%
\begin{eqnarray}
- \frac{d}{dt}\left( \frac{\partial c}{\partial \dot{y}}^H \Lambda \right)
+  \frac{\partial c}{\partial y}^H \Lambda + \frac{\partial g}{\partial y}^H
...
\end{equation}
%
where we have substituted $\partial y_0 / {}\partial p$ for $\partial
y / {}\partial p$ in the last term.
...
(\ref{rythmos:apdx:eqn:sens:adj-trans}) and its final condition
(\ref{rythmos:apdx:eqn:sens:adj-trans:fc}) and noting that $y(t_0) =
