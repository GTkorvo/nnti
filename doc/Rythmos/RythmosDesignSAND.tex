...
(\ref{rythmos:eqn:sens:f})--(\ref{rythmos:eqn:sens:f:ic}) at points
...
& = & 0, \; \mbox{for} \; t \in \left[ t_0, t_f \right],
...
As the adjoint is being computed, the integral in
%
\begin{equation}
\Jac{\hat{d}}{p}^T
=
...
%
\subsubsection{Reverse-time adjoint}
%

In order for general purpose forward time stepping algorithms and software to
be used to solve for the adjoint, the adjoint equation in
(\ref{rythmos:app:eqn:sens:adj})--(\ref{rythmos:app:eqn:sens:adj:fc}) must be
stated in reverse time.  Reverse time $\bar{t}\in[0,t_f-t_0]$ is defined so
that $t = t_f - {}\bar{t}$.  Since the adjoint equation in
(\ref{rythmos:app:eqn:sens:adj}) involves fixed operators and is linear in the
adjoint varaibles, this change in the meaning of time only affects the
definition of $\frac{d}{dt}$ first term.  Reverse time means that $dt =
-d\bar{t}$ which gives
%
\begin{eqnarray}
\frac{d}{d\bar{t}}\left( \Jac{f}{\dot{x}}^T \breve{\Lambda} \right)
+ \Jac{f}{x}^T \breve{\Lambda} - \Jac{g}{x}^T
& = & 0, \; \mbox{for} \; \bar{t} \in \left[0, t_f-t_0 \right],
\label{rythmos:app:eqn:sens:adj-rev} \\
\left.\left( \Jac{f}{\dot{x}}^T \breve{\Lambda} \right)\right|_{\bar{t}=0}
& = & \left. \Jac{h}{x}^T \right|_{x=x(t_f)},
\label{rythmos:app:eqn:sens:adj-rev:fc}
\end{eqnarray}
%
where $\breve{\Lambda}=\Lambda$ and $\dot{\breve{\Lambda}} =
d(\breve{\Lambda})/d(\bar{t}) = - d(\Lambda)/d(t) = -\dot{\Lambda}$.

{}\textbf{Warning!} One must be careful when using this formulation to
remember that while $\breve{\Lambda}=\Lambda$ is the orignial forward-time
adjoint solution, the reverse-time time derivative $\dot{\breve{\Lambda}}$ is
the {}\textit{negative} of the forward-time time derivative
$\dot{\breve{\Lambda}} = -\dot{\Lambda}$.  Other than that, the reverse-time
adjoint formulation is very straight forward and allows forward-stepping time
integration algorithms and software to be used off-the-shelf.
...
(\ref{rythmos:app:eqn:sens:adj-rev}), as stated in reverse-time $\bar{t}$
reduces to
%
\begin{eqnarray}
\Jac{f}{\dot{x}}^T \dot{\breve{\Lambda}}
+ \Jac{f}{x}^T \breve{\Lambda} - \Jac{g}{x}^T
& = & 0, \; t \in \left[ t_0, t_f \right],
\label{rythmos:app:eqn:sens:adj-const-mass} \\
\left.\left( \Jac{f}{\dot{x}}^T \breve{\Lambda} \right)\right|_{t=t_f}
& = & \left. \Jac{h}{x}^T \right|_{x=x(t_f)},
\label{rythmos:app:eqn:sens:adj-const-mass:fc}
\end{eqnarray}
%
where $\dot{\breve{\Lambda}}=-\dot{\Lambda}=
-d(\Lambda)/d(t)\in\mathcal{F}|\mathcal{G}$.

This reverse-time adjoint DAE can be cast in the standard form
...
\bar{x} =
{\bmat{c}
\breve{\lambda}_{(0)} \\ \vdots \\ \breve{\lambda}_{(n_g-1)}
\emat}
\in \mathcal{F}^{n_g}
\end{equation}
%
consisting of the columns $\breve{\lambda}_{j}$ of $\breve{\Lambda}$ and then
defining the associated composite DAE function
%
\begin{equation}
\bar{f}(\dot{\bar{x}},\bar{x},\bar{t}) =
{\bmat{c}
%
\Jac{f}{\dot{x}}^T \dot{\breve{\lambda}}_{(0)}
+ \Jac{f}{x}^T \breve{\lambda}_{(0)} - \Jac{g_{(0)}}{x}^T
%
\\ \vdots \\
%
\Jac{f}{\dot{x}}^T \dot{\breve{\lambda}}_{(n_g-1)}
+ \Jac{f}{x}^T \breve{\lambda}_{(n_g-1)} - \Jac{g_{(n_g-1)}}{x}^T
%
\emat}
...
matrix $\tilde{W} = \bar{\alpha} (\jac{f}{\dot{x}}) + {}\bar{\beta}
(\jac{f}{x})$ which is needed by the implicit forward and direct direct
...
these objects also need to be supported (see
...
such as is the case in CVODES and IDAS.


%
\subsubsection*{Evaluation of Non-augmented adjoint equation residual}
%

Given the basic operator $\tilde{W} = {}\bar{\alpha} (\jac{f}{\dot{x}}) +
{}\bar{\beta} (\jac{f}{x})$ that is needed in the defintion of $\bar{W}$ in
(\ref{rythmos:eqn:adj-sens-daes-jac}) and $\jac{f}{\dot{x}}$ as a separate
operator, one can compute the multi-vector adjoint residual in
(\ref{rythmos:app:eqn:sens:adj-const-mass}).  To see this, first note that
%
\begin{equation}
\Jac{f}{x}^T = \frac{1}{\bar{\beta}} \tilde{W}^T - \frac{\bar{\alpha}}{\bar{\beta}} \Jac{f}{\dot{x}}^T.
\label{rythmos:eqn:sub-d_f_d_x_adj}
\end{equation}
%
Substituting (\ref{rythmos:eqn:sub-d_f_d_x_adj}) into
(\ref{rythmos:app:eqn:sens:adj-const-mass}) and rearranging yields
%
\begin{equation}
\Jac{f}{\dot{x}}^T \dot{\breve{\Lambda}}
+ \Jac{f}{x}^T \breve{\Lambda} - \Jac{g}{x}^T
=
\Jac{f}{\dot{x}}^T \left( \dot{\breve{\Lambda}} - \frac{\bar{\alpha}}{\bar{\beta}} \breve{\Lambda} \right)
+ \frac{1}{\bar{\beta}} \tilde{W}^T \breve{\lambda}
- \Jac{g}{x}^T.
\end{equation}
%
This residual evaluation requires two adjoint linear operator applications:
one for $(\jac{f}{\dot{x}})^T$ and one for $(\tilde{W})^T$.

The so-called mass matrix $\jac{f}{\dot{x}}$ is computed by calling
{}\texttt{evalModel(...)} on the underlying forward state model by setting
$\alpha=1.0$ and $\beta=0.0$ and evaluating $W$.  In the most general case,
two calls to the underlying forward model's {}\texttt{evalModel(...)} function
are required in order to compute $\jac{f}{\dot{x}}$ and $\tilde{W} =
{}\bar{\alpha} (\jac{f}{\dot{x}}) + {}\bar{\beta} (\jac{f}{x})$.  However,
when $\jac{f}{\dot{x}}$ is constant, which is the case in many models, it can
be computed once and be reused multiple times.  Typically, however, since
$\bar{\alpha}$ and $\bar{\beta}$ are likely to change from one time step to
the next, $\tilde{W}$ typically needs to be recomputed for each time step even
if $\jac{f}{\dot{x}}$ and $\jac{f}{x}$ are constant.
...
When the mass matrix $\jac{f}{\dot{x}}$ is a function of $t$, then the
reverse-time adjoint equations in
(\ref{rythmos:app:eqn:sens:adj-rev})--(\ref{rythmos:app:eqn:sens:adj-rev:fc})
can be transformed into the {}\textit{reverse-time augmented adjoint
equations} which take the form
%
\begin{eqnarray}
\frac{d}{d\bar{t}}\left( \hat{\Lambda} \right)
+ \Jac{f}{x}^T \Lambda - \Jac{g}{x}^T
& = & 0, \; t \in \left[ t_0, t_f \right],
...
\end{equation}
%
where $\hat{\alpha}=-\bar{\beta}$, $\hat{\beta}=-\bar{\beta}^2/\bar{\alpha}$,
and $\hat{\gamma}=-\bar{\beta}/\bar{\alpha}$.
...
%
\[
\hat{W} = \hat{\alpha} \Jac{f}{\dot{x}}^T + \hat{\beta} \Jac{f}{x}^T
\]
%
...
but requires twice the $O(n_f)$ time-step storage since both $\hat{\Lambda}$
