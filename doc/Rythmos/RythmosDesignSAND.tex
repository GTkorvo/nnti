...
%
\begin{equation}
\Jac{\bar{f}}{\bar{x}} \Delta \bar{x} = - \bar{f}(\bar{x}_l)
\end{equation}
%
where $\bar{x}_l$ is the current candidate solution of the nonlinear equations
(which also defines the point where $\jac{\bar{f}}{\bar{x}}$ is evaluated) and $\Delta \bar{x} =
\bar{f}_{l+1} - \bar{f}_l$ is the update direction.  Line-search Newton methods then
define an update to the solution along the direction $\Delta \bar{x}$.  The
essential functionality needed to perform a Newton-like method are the the
abilities to evaluate the nonlinear residual $\bar{x} {}\rightarrow \bar{f}$ and to
(approximately) solve linear systems involving the Jacobian matrix
$\jac{\bar{f}}{\bar{x}}$.  For each type of implicit time integration method, we will
show, if possible, how to perform solves with $\jac{\bar{f}}{\bar{x}}$ by performing
...
(\ref{rythmos:eqn:W_bdf}) may not necessarily exactly represent $\jac{\bar{f}}{\bar{x}}$
and $\bar{x}$ and $\bar{f}$ may not simply lie in the vector spaces $\mathcal{X}$ and
...
equations (\ref{rythmos:eqn:f_bar}) and with unknown solution variables $\bar{x} =
...
%
\begin{equation}
\bar{f}_i(\bar{x}) = f\left( \dot{x}_i, x_k + \Delta t \sum_{j=0}^{p-1} a_{ij} \dot{x}_j,
...
$\dot{x}(t_k + c_i \Delta t)$ called {}\textit{stage derivatives} and $\bar{x} = [
...
It is clear how to form the residual for the fully coupled system for $\bar{f}(\bar{x})$ in
...
block $\jac{\bar{f}_i}{\bar{x}_j} = {}\jac{\bar{f}_i}{\dot{x}_j}$ of the full Jacobian
$\jac{\bar{f}}{\bar{x}}$ is represented as
%
\begin{equation}
\Jac{\bar{f}_i}{\dot{x}_j}
= \Jac{f}{\dot{x}}
...
that $\jac{\bar{f}_i}{\bar{x}_j} = {}\jac{\bar{f}_i}{\dot{x}_j}$ in
...
operator matrix $\jac{\bar{f}}{\bar{x}}$, it is easy to see how to use the general matrix
...
$\jac{\bar{f}}{\bar{x}}$ but this is not something that can be expected for most
applications.
...
Jacobian matrix $\jac{\bar{f}}{\bar{x}}$.  For these types of methods, the nonlinear
...
give rise to a lower triangular Jacobian $\jac{\bar{f}}{\bar{x}}$ (and hence are also
...
implicit nonlinear system $\bar{f}(\bar{x})=0$ that arises and a Newton-like method would
...
nonlinear time step equations $\bar{f}(\bar{x})=0$ in (\ref{rythmos:eqn:f_bar}) or in the
...
= \frac{1}{\Delta t} M + \Jac{\bar{f}}{x}.
\]
%
To see how to use $\bar{f}(x_{k+1},x_k,t_k,\Delta t)$ to compute
...
These leads to the evaluation of $f(\ldots)$ in terms of $\bar{f}(\ldots)$ as
%
\[
f(\dot{x},x,t)
= \bar{f}(x, x - \Delta t \dot{x},t - \Delta t, \Delta t)
\]
%
...
implement the functions $\bar{f}(x_{k+1},x_k,t_k,\Delta t)$ and
