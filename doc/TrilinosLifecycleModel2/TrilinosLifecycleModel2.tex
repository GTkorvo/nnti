...
development, integration and on-going support of support libraries for
computational science and engineering applications.  Trilinos is
composed of a set of different packages with a wide range of
capabilities including basic distributed memory linear algebra, load
balancing, discretization support, and wide variety of nonlinear
...
(*) Add specific bullets for the motivation for the lifecycle model:
a) better communicate maturity with users, b) help to idendify and
foster SE practicies that will help Trilinos devleopers be more
productive and create higher quality code.

(*) Change from ``Purely Experimental'' to ``Exploritory'' and state
that Exploritory code should be thrown away by default when moving to
Research Stable.

(*) Remove ``Stable'' from the various phases.  (This will get rid of
Production Maintenance Stable being PMS.)

(*) Change the name Primary Stable (PS) and Secondary Stable (SS) to
Tier 1 (T1) and Tier 2 (T2) to avoid confusing these with maturity
levels.

(*) Add the official ability for packages to ``opt out'' of the
lifecycle model.  Define the maturity level ``Unspecified Maturity''
for packages that don't want to assign a maturity level.

(*) Add discussion of using the the legacy code change process
(i.e. cover, change, refactor, repeat) to the discussioin of
grandfathering existing packages.  Then, add the official prefix
``Grandfathered'' to the maturity levels (e.g., Grandfathered
Production Growth (GPG), Grandfathered Production Maintenance (GPM),
etc.).  Package developers that have packages that are grandfathered
in must promise to sue the legacy code change process or they have to
stick with the opt out Unspecified Maturity level.
...
\fbox{
\begin{minipage}[b]{0.45\textwidth}
{}\textbf{Side Note:} It is also possible to take Purely Experimental
software and turn it into Production Growth Stable software but it
would require a large refactoring effort driven by an intense unit and
verification test development effort.  The large cost needed to get
Purely Experimental code to the standard of quality needed for
Research Stable code (the precondition for entering the Production
Growth Stable phase) is usually not undertaken and therefore software
that is not developed from the beginning with strong testing with
continuous refactoring to maintain clear design and code typically
never achieves the properties necessary for self-sustaining software.
\end{minipage}
} %fbox
\end{center}
\end{floatingfigure}
...
PRODUCTION\_MAINTENANCE\_STABLE} and all software that does not
...
{}\ttt{Trilinos\_ENABLE\_MINIMUM\_MATURITY\_LEVEL =
PRODUCTION\_MAINTENANCE\_STABLE}, only the one Production Maintenance
Stable subpackage would be enabled.  However, when Trilinos was
configured with {}\ttt{Trilinos\_ENABLE\_MINIMUM\_MATURITY\_LEVEL =
RESEARCH\_STABLE}, then the Production Maintenance Stable, Production
