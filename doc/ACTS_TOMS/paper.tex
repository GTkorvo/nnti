...
\subsection{Trilinos Package Concept}

The fundamental concept in Trilinos is the {\it package}. Trilinos
...
tracking.  There are also guidelines for adding new packages 
to Trilinos, and it is worth noting here that Trilinos is the 
first and only solver framework to formalize the process of 
integrating new solver technology into the framework. 
...
of documented interfaces, without modifying PETSc or Trilinos source code.  
...
pieces of software.  However, the CCA uses a much more advance runtime
...
MTL is a major problem since a number of C++ compilers do not 
support some of
...
primary difference is that MTL executes on serial machines only.  
...
focus on arrays and overload operator syntax, both of which make
grid-based calculations and explicit methods easy to implement.  In
fact, much of its terminology uses grid concepts.  POOMA also focuses
...
using POOMA objects.
Trilinos can be used for grid-based computations, but it primarily focuses on
irregular, unstructured computations.  Trilinos makes no assumptions
about regularity and, although several Trilinos packages are dedicated
to basic linear algebra computations, the primary focus of Trilinos is
on advanced solver algorithms.

\subsection{Trilinos and Other Solver Libraries}
Trilinos is different than other solver projects in that it both
provides significant new solver capabilities as independent packages
and also provides an explicit, documented modular architecture that 
facilitates interoperability.  This allows Trilinos to easily use
other solver libraries and to also provide, via any single Trilinos
package, solver capabilities to other libraries.

\section{Services Provided by Trilinos Framework}
\label{sect:TrilinosServices}

Trilinos provides a variety of services to mathematical library
developers.  First and foremost, the Trilinos package architecture
allows the introduction of a new package without any interdependencies
between a new package and the rest of Trilinos.  
