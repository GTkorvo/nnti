...
\end{equation}
%
For $Q\in\RE^{n\times{}n}$ to be full rank, $E\in\RE^{n\times{}n}$ must be full
rank.  Here we define a Euclidean vector space as one where
$Q=E=I\in\RE^{n\times{}n}$.
...
Thyra also defines linear operators which linearly map vectors from one vector
space to another.  The defintion of a linear operator is strongly influenced
by the defintion of the scalar product associated with a vector space and
whether the mapping is between Euclidean vectors $y = A x$ or between
coefficient vector $\tilde{y} = \tilde{A} {}\tilde{x}$ (see [???] for more
details).  In Thyra, vectors are a specialization of linear operators and
therefore every vector is also a linear operator.  Therefore, when one writes
%
\begin{equation}
z = x^H y
\end{equation}
...
of the vectors in that they can be through of as the Eucliean linear operators
...
assumptions about where or how those coefficients are stored.  A fully general
ANA can make no assumptions about how vectors are stored or layed out in
memory, only that those coefficients do exist and that these coefficients can
be exposed to reduction and transformation operators (see [???]).  While
accessing the elements of a vector is ill advised in a general ANA, the Thyra
vector interface supports acquiring direct views of any range of vector
