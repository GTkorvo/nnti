...
Thyra ANA Operator/Vector Interfaces, we eliminate the many-to-many dependency
...
All Thyra vectors belonging to a vector space are representable as an array of
scalar coefficents $\tilde{x}\in\RE^n$ and a (non-unique) basis
...
{}\texttt{Thyra::\-Vector\-Base}.  Vectors are created from the vector space
using the ``Abstract Factory'' design pattern.

In addition to vectors and vector spaces, Thyra also defines linear operators
which linearly map vectors from one vector space to another.  The defintion of
a linear operator is strongly influenced by the defintion of the scalar
product associated with a vector space and whether the mapping is between
Euclidean vectors $y = A x$ or between coefficient vector $\tilde{y} =
\tilde{A} {}\tilde{x}$ (see [???] for more details).  In Thyra, vectors are a
specialization of linear operators and therefore every vector is also a linear
operator.  Therefore, when one writes
%
\begin{equation}
z = x^H y
\end{equation}
...
defined by the scalar product matrix $Q\in\RE^{n\times{}n}$.  In Thyra, it is
...
of the vectors in that they can be thought of as the Eucliean linear operators
that only update the coefficient vectors or as linear operators that act
directly on vector coefficinets.

The Fundamental Thyra ANA Operator/Vector Interfaces require that every Thyra
vector be represented as a finite-dimensional set of scalar coefficents and
that the scalar product $<\tilde{x},\tilde{y}>$ be equivalent to the two-sided
application some finite-dimensional symmetic positive definite matrix
...
infinite dimensional or that they admit more general implementations as it
allowed by infinite dimensional Hilbert spaces [???].  While every Thyra
vector must be stored as a set of scalar coefficients the interfaces make no
assumptions whatsoever about where or how those coefficients are stored.  A
fully general ANA can make no assumptions about how vectors are stored or
layed out in memory, only that those coefficients do exist and that these
coefficients can be exposed to reduction and transformation operators (see
[???]).
...
Again, thyra does not try to hide that a vector is simply a set of scalar
coefficients associated with some basis.  Thyra does not even really try to
keep the client from accessing the actual coefficients of the vectors since it
can access them with an RTOp object anyway.  What Thyra does do is to abstract
where the vector coefficients live and what native data structure is used to
hold the coefficients.  None of vector coefficients may even be directly be
head in main memory in the process where the ANA is running but there is aways
a (perhaps very inefficient) mechanism to get a view of them.  However, ANAs
that want to be maximally general and efficient should not try to access the
vector elements explicitly through and many ANAs do not.  However, there are
perfectly reasonable ANAs that do need to access the explicit vector
coefficients for vectors from certain vector spaces (such as vectors in the
design space in some reduced-space optimization algorithms) and Thyra provides
efficient and yet 100\% general ways to access these coefficients.  Every
Thyra vector space also has a finite dimension that is accessible as a
property of the vector space.

While Thyra requires vectors to be finite dimensional and the coefficients of
vectors must be accessible (if to none other than to RTOps), it does allow
...
wrapper convenience function in the ANA support code collection (see ???).
The set of operations is also easily extensible.  Every
