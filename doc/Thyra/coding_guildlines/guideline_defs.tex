...
\newcommand{\NOSFTemplateFiles}{\stepcounter{Thyra_NOSF_counter}
\textbf{NOSF~\arabic{Thyra_NOSF_counter}}:
{}\textit{Partition template code into the files
{}\ttt{FileName\_decl.hpp}, {}\ttt{FileName\_def.hpp},
{}\ttt{FileName.hpp}, and {}\ttt{FileName.cpp} to allow for both
implicit and explicit instantiation and handling of circular type
references}}
...
\textbf{GCG~\arabic{Thyra_GCG_counter}}:
{}\textit{Avoid the use of raw C++ pointers in all but the
lowest-level code}}
...
\textbf{GCG~\arabic{Thyra_GCG_counter}}:
{}\textit{Use {}\ttt{Teuchos::Ptr} as function arguments in the place
of raw C++ pointers to single objects where no persisting association
exists}}
...
\textbf{GCG~\arabic{Thyra_GCG_counter}}:
{}\textit{Use {}\ttt{Teuchos::ArrayView} as function arguments in the
place of pointers into raw arrays or other container classes where no
persisting association exists and the array does not need to be
resized}}
...
value to establish semi-persisting associations, never a raw C++
reference to be used as a semi-persisting association}}
...
\textbf{GCG~\arabic{Thyra_GCG_counter}}:
{}\textit{Prefer to explicitly define template arguments in a template
function call to avoid protability problems and enable implicit
covnersions of input arguments}}

\newcommand{\GCGTeuchosAs}{\stepcounter{Thyra_GCG_counter}
\textbf{GCG~\arabic{Thyra_GCG_counter}}:
{}\textit{Use the template function
{}\ttt{Teuchos::as<T\_to>(T\_from)} for all conversion of data types
that may result in loss of precision or in an incorrect conversion}}
...
{}\ttt{SomeClass.hpp}, and {}\ttt{SomeClass.cpp} to support explicit
instantiation, minimize recompilation, and avoid problems in mutually
dependent (i.e. circular) code}}
...
independent of any single concrete implementation}}
...
\textbf{DOX~\arabic{Thyra_DOX_counter}}:
{}\textit{Provide typical pre- and post-conditions along with the
...
and so refer to the return object as {}\ttt{returnVal}}}

\newcommand{\DOXPreferPostconditionsInParam}{\stepcounter{Thyra_DOX_counter}
\textbf{DOX~\arabic{Thyra_DOX_counter}}:
{}\textit{Prefer specifying post-conditions for output arguments in
their {}\ttt{$\backslash$param} field; otherwise specify their
post-conditions in the 'Post-conditions' list}}
...
Post-conditions, {}\ttt{$\backslash$returns}, then detailed
documentation; omitting those that do not apply}}
