...
In rest of this document is organized as follows.  Naming conventions are
...
{}\textbf{TCDDG}.  For example, the first naming convention guideline can be
refereed to as {}\textbf{TCDDG NC 1}.  In this way, these short precise
alpha-numeric designation such as {}\textbf{TCDDG NC 3} can be used in code
...
{}\texttt{Teuchos\-::Fancy\-OStream} object instead of a bare
{}\texttt{std\-::ostream} objects to more easily produce formatted output}: A
{}\texttt{Teuchos\-::Fancy\-OStream} class object can wrap any
{}\texttt{std\-::ostream} and helps to produce structured indented and to create
more readable output in an SPMD program (even when every processor produces
output).

{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Derive from {}\texttt{Teuchos\-::Describable} and implement the
{}\texttt{description()} and {}\texttt{describe()} to allow clients to print
the current state of an object}: The {}\texttt{Teuchos\-::Describable} is the
appropriate way to allow clients to print the current state of an object in a
flexible way.  The verbosity of the output is controlled by an enum parameter.

{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Derive from {}\texttt{Teuchos\-::Verbose\-Object} and print to
...
{}\item\stepcounter{Thyra_FSC_counter}\textbf{FSC \arabic{Thyra_FSC_counter}}:
{}\textit{List sort function prototypes on one line and longer protypes
...
also be applied to function definitions and function calls.
  ...
  {}\item\stepcounter{Thyra_FSC_counter}\textbf{FSC \arabic{Thyra_FSC_counter}}:
{}\textit{List short function prototypes on one line if possible}: For
  example,
  ...
  {}\item\stepcounter{Thyra_FSC_counter}\textbf{FSC \arabic{Thyra_FSC_counter}}:
{}\textit{For longer prototypes, indent arguments on continuation lines
...
structures.
  ...
  {}\item\stepcounter{Thyra_FSC_counter}\textbf{FSC
  \arabic{Thyra_FSC_counter}}: {}\textit{Formatting if/else if/else
  ...
  {}\item\stepcounter{Thyra_FSC_counter}\textbf{FSC
  \arabic{Thyra_FSC_counter}}: {}\textit{Formatting switch/case statements}:
  ...
  {}\item\stepcounter{Thyra_FSC_counter}\textbf{FSC \arabic{Thyra_FSC_counter}}:
{}\textit{Formatting for and while loops}: The two recommended styles
