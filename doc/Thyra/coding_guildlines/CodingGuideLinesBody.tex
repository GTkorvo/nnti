...
in the book and tailor them for numerical libraries.  The zeroth item (first
item, zero based) ``Don't seat the small stuff'' intentionally avoids specific
...
for adopting consistent coding style and some of these are outlined in
...
naming conventions and code formatting that provide for code uniformity and
...
without exception, should use include guards {}\cite[Item
...
{}\item\textit{Prefer enum arguments to bool objects as formal function
...
problems in mutually dependent code}: ??? ToDo: Merge this with the above item
on layout of files for implicit and explicit template instantiation once we
figure out how to do this with explicit and implicit instantiation ???
...
and long prototypes are given.  These guidelines seek to produce function
prototypes that are fairly tight (i.e. not too much whitespace explosion), are
robust to modifications, and keep code inside of the 80th character column.
This indentation style can (and should) also be applied to function
...
{}\item\textit{Placement of braces and indentation of control structures}: Two
basic styles of brace placement and indentation in control structures are
recommend here.  The first general style is a modification of the K\&R
style{}\cite{ArtisticStyle} where the brace comes immediately after the
...
pure block'' format that he sites the ANSI styles as violating this principle
...
single line, prefer the ``Linux'' style.

Below, the application of the modified K\&R style and the Linux styles are
