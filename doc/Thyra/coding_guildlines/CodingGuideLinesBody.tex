...
the guildelines in the book ``C++ Coding Standards'' by Sutter and
Alexandrescu {}\cite{C++CodingStandards05} (the 101 items are outlined in
Appendix~\ref{sec:cpp_coding_standards}) and the memory management approach
described in {}\cite{TeuchosMemoryManagementGuide}.  The guidelines in this
document are specifically designed to address the development of
object-oriented numerical C++ libraries and to utilize the tools in the
Trilinos package {}\texttt{Teuchos}.  While the main purpose of of this
document is to define guidelines for Thyra software (for both interfaces and
adapters), it is also general enough to be applied to many other projects
...
arguments are outlined in Appendix~\ref{sec:arguments-for-consistent-style}.
Therefore, one of the purposes of this document is to suggest reasonable and
minimal guidelines for naming conventions and code formatting that provide for
enough code uniformity to facilitate collaborative code development, code
...
Appendix~\ref{sec:summary_guidelines}.  Similarly, the 101 guidelines from
{}\cite{C++CodingStandards05} are listed in
Appendix~\ref{sec:cpp_coding_standards} along with specifying which items are
amended or invalidated by the guidelines in the current document.
Appendix~\ref{sec:misc_amendments} discusses the items from
...
described in {}\cite[Item 59]{C++CodingStandards05}.
...
to define the scope of the variable and differentate that name from a local
varaible or a member function that may otherwise result and result in
``shadowing'' which causes protability problems on some compilers.


{}\textit{Exception}: Public date members in simple C++ structs (i.e.\ where
no invarients need to be maintained) should not contain underscores.  For
example:
...
encapculation unit of code together which makes searching the encapsulation
unit easier.
...
{}\item\NOSFTemplateFiles: The template delcaration code and inline function
...
runtime checking that result in exceptions being thrown and excellent error
...
greater flexibilty in how a class object is initialized, simplifies
...
Note that views of concrete classes do not have ot use ``generalized
...
(*) Only return ArrayView objects in cases where the overhead of an ArrayRCP
object would be too great.  However, in debug mode, misuse would be detected.
...
results in stronger runtime checking.  While {}\texttt{Teuchos::\-Array} gets
...
{}\item\GCGTeuchosParamterListValidation: All user parameters and sublists
...
approaches, when a user mispells a parameter or sublist, uses the wrong type
...
responsible for validating their own parameters and sublists, and not those of
other objects that they hold sublists for.
...
{}\item\GCGTemplateHeaders: Breaking up templated C++ into the four files
...
of recompilations, and c) handling of any and all types of cicular
dependancies in declarations and definitions (same as are allowed with
non-templated C++ code).

{}\textbf{ToDo:} Give the code examples and explain when to include the
XXX\_decl.hpp versions of header files and when you can just include the
XXX.hpp versions.
...
The guidelines for specifying local variables and data membes, passing objects
to and from functions, and returning objects from functions given in
{}\cite{TeuchosMemoryManagementGuide} are sumarized in
