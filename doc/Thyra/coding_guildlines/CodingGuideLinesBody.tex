...
starting with {}\textbf{TCDG}.  For example, the first naming convention
guideline can be refereed to as {}\textbf{TCDG NC 1}.  In this way, these
short precise alpha-numeric designation such as {}\textbf{TCDG NC 3} can be
used in code reviews as short-hand references to specific guidelines.
...
{}\item\stepcounter{Thyra_NC_counter}\textbf{NC \arabic{Thyra_NC_counter}}:
{}\textit{C++ class member functon names}: Names for class member functions
...
object but is always default initialized to NULL.  In debug mode, it throws
...
{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Use Teuchos::RCP for memory management of single
dynamically allocated objects and for handling persisting
{}\cite{RCPBeginnersGuide} associations}: Replace all references to
...
{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Prefer Teuchos::Array to std::vector as a contiguous general purpose
data container}: While {}\texttt{Teuchos::\-Array} gets all of its real
...
{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Use Teuchos::ArrayRCP for memory management of dynamically allocated
objects stored in contiguous arrays of data and for persisting associations
involving contiguous arrays}: Note that {}\texttt{Teuchos::ArrayRCP} does
...
{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Prefer Teuchos::Map to std::map as a general associative container}:
While in optimized mode, {}\texttt{Teuchos\-::Map} is equivalent to
{}\texttt{std\-::map}, in debug mode {}\texttt{Teuchos\-::Map} uses a
simplified data-structure with fully checking of access and iterators.  This
class uses {}\texttt{Teuchos\-::Array}, {}\texttt{Teuchos::List} and others to
build a fully checked (but slower) implementation.
...
{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Structure the includes in template header files to avoid
...
{}\item\stepcounter{Thyra_GCG_counter}\textbf{GCG \arabic{Thyra_GCG_counter}}:
{}\textit{Include only standard C++ headers, not standard C headers, and avoid
{}\texttt{using namespace std} directives}: Only include the C++
{}\texttt{<cX>} versions of the standard C {}\texttt{<X.h>} headers.  For
example, include {}\texttt{<cmath>}, {}\texttt{<cstdlib>}, and
{}\texttt{<cassert>} instead of {}\texttt{<math.h>}, {}\texttt{<stdlib.h>},
and {}\texttt{<assert.h>}.  Avoid all uses of {}\texttt{using namespace std}
...
compile-time and run-time checking, and is as auto-documenting as possible.  A
key component to this specification is that no raw C++ pointers are used.  Raw
...
When passing a single object to a function, four different properties have to
be specified: 1) does the object use value semantics or reference semantics,
2) is the object changeable (const) or changeable (nonconst), 3) is this
...
C++ code except for whether an argument or object is required or optional and
therefore must be documented in the function's interface specification (i.e.\
in the doxygen documentation {}\texttt{\\param} field).

An array of value objects just stores the objects themselves and passes them
through an {}\texttt{Teuchos\-::ArrayView<T>} object.  An array of reference
objects, however, can not directly store the objects themselves and instead
must store an array to (smart) pointers to the reference objects.
...
value-type and reference-type objects, respectively.
