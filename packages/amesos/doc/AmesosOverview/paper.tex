...
\author{Marzio Sala, Ken Stanley and M. A. Heroux}
...
library~\cite{alpatov97plapack}, there is no single direct solver library of
...
Using libraries can be a non-trivial task, as its
developers often include a wide variety
...
Of course, a simple but inefficient library is of no interest; therefore the
...
set of interest, and this can be done only if the interface  
...
the codes and a natural reluctance to change successful projects discourage any
effort of rewriting unless absolutely necessary.
Since a new library 
(or a new version of a given library) may require a new matrix format or
distribution, or new calling conventions, application developers may simply decide to continue
using the interface already developed.
\end{itemize}

This article shows that it is possible to overcome all these problems by 
resorting to object-oriented (OO) design and
...
formats, data layout, and calling conventions. Our desiggn is based on the
...
%
As regards the parallel computing mode, 
...
This approach is followed by several scientific libraries, see
...
suitable ordering and data structures for an efficient factorization;
\item[(S2)] a numerical factorization phase where the values of $L$, $U$ and
$L$ are computed;
\item[(S3)] a solve phase where $L$, $U$ and $D$ used to solve the system using
...
since $A$ is sparse, $L$ and $U$ are still sparse, though they may have some
fill-in, i.e. non-zero entries which are zero in $A$. Therefore, the
factorization is performed on $PAQ$.  
...
We have just mentioned few reasons that explained why
...
Despite these differences, all solvers are accessed within 
a given application by using a sequence of
...
\end{enumerate}
Steps (A1), (A2) and (A3) are application-dependent and will not be discussed
here; instead, our aim is to standardized 
steps (S0/1), (S2) and (S3)  by
adding an intermediate layer between the application and the
direct solver libraries. We introduce a set of abstract classes, which will be
used to define interfaces to the data layout of distributed objects, 
vectors, matrices, linear system, and the solver.
The design is here reported as a set of C++ classes, but the concepts are more general
...
layout of distributed vectors and matrices. 
...
each interface can be enabled or disabled at configure time. Users can take
...
however by using the factory class, other
...
available, the application can use the factory class to access any of the supported
libraries.
...
\item The actual data storage format of the linear system matrix becomes inessential.
...
vectors into the library's format. For very large matrices, this can constitute a
...
Despite these (minor) problems, we believe that the presented set of interfaces
...
support single precision and complex arithmetics (by using templates, for
                                                  example), and a templated
version of
of Interfaces~\ref{int:map}--\ref{int:asi} is under development.

%-----------------------------------------------------------------------------%
\section*{Acknowledgments}
%-----------------------------------------------------------------------------%

The authors wish to thank R.~Hoekstra, T.~Davis and I. Duff for
