...
\cite{amestoy01analysis,gupta01recent} and the references therein.
...
Compared to the factorization, the serial cost of this a solve phase
...
here; instead, our aim is to standardized steps (S0/1), (S2) and
...
\end{itemize}
\end{interface}
Interface~\ref{int:lp} can be easily extended to standardize
operations like scaling or reordering.
...
\amesos\ provides and abstraction for Interface~\ref{int:asi}, as
well as all its concrete implementations (for a little more than
13,000 code lines, including comments and Doxygen~\cite{Doxygen}
...
Figure~\ref{fig:example}. Although this particular example requires
MPI, \amesos\ can be compiled with or without support for MPI.
(Clearly, distributed solvers are available only when compiled with
MPI support.) The only \amesos\ include file is \verb!Amesos.h!,
