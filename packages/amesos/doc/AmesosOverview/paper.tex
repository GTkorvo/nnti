...
consult the specialized literature, for example~\cite{duff86direct} and the
references therein. 
...
\end{itemize}
Some codes combine phases (S1) and (S2). Phase (S2) is usually requires the most
...
%
Oblio       & $\bullet$ & -- & $\bullet$ & ?? & left, right, multifrontal &
\cite{dobrian00oblio} \\
%  
PARDISO     & $\bullet$ & S & -- & C & left/right supernodal  & \cite{oskl:04-etna,sg:04-fgcs} \\
%
PaStiX      & $\bullet$ & D &  ?? & -- & left-looking supernodal &
\cite{henon00pastix} \\
%
...
WSMP & $\bullet$ & S/D & -- & -- & multifrontal & \cite{gupta02improved} \\
...
solvers; see also~\cite[Section 6.7]{dongarra98numerical} and
\cite{davis05summary} for a broader overview of available solvers, and
\cite{hsl06collection} for other direct solver subroutines.
...
steps (S0)--(S1) of Section~\ref{sec:introduction}:
\begin{enumerate}
\item[(A1)] Definition of the sparsity pattern of the linear system matrix;
\item[(S0/S1)] Computation of the symbolic factorization;
...
int main(int argc, char *argv[]) 
{
  ...
  Epetra_MultiVector* x = <create solution vector here, Int. 3.2>;
  Epetra_MultiVector* b = <create right-hand side here, Int 3.2>; 
  Epetra_CrsMatrix* A = <create matrix here, Int 3.3>;
