...
under typical conditions. We report examples of applications using the
presented interfaces, and we comment
on the advantages and limitations of the design.
\end{abstract}
...
easy to solve. The process of computing these three matrices is called {\sl
...
computation is some variant of Gaussian elimination method~\cite{REFME}. 
...
choice for for (distributed) sparse linear systems.  Since no ``gold
...
consult the specialized literature, for example~\cite{REFME} and the
references therein. 

\smallskip

Sparse direct solvers are complex codes which use a number of
strategies to reduce memory and time costs.  
...
Table~\ref{tab:packages} reports some of the publicly available direct
solvers. A closer look at these solvers reveals the following major
diffrences:
\begin{itemize}
...
steps:
\begin{enumerate}
\item[(S1)] Definition of the sparsity pattern of the linear system matrix;
\item[(S2)] Computation of the symbolic factorization;
\item[(S3)] Definition of the values of the linear system matrix;
\item[(S4)] Computation of the numeric factorization;
\item[(S5)] Definition of the values of the right-hand side;
\item[(S6)] Solution of the linear system.
\end{enumerate}
A flowchart of processes (S1)-(S6) is reported in Figure~\ref{fig:flowchart}.
While steps (S1), (S3) and (S5) are application-dependent, steps (S2), (S4) and
(S6) can be abstracted to define three phases: a {\sl symbolic
...
Our aim is to standardize steps (S2), (S4)
and (S6) to allow a transparent and consistent usage
...
layout of distributed vectors and matrices. The class {\tt Vector} defines a
...
int main(int argc, char *argv[]) 
{
  ...
  Epetra_CrsMatrix* A = <create matrix here>;
...
implementations. The Standard Template Library (STL)~\cite{stl} is used
