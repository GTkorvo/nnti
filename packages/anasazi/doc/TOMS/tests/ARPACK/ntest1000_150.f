...
c
      ...
      parameter         (maxn=1000000, maxncv=150, maxnev=145, ldv=maxn)
c
c     %--------------%
c     | Local Arrays |
c     %--------------%
...
c
      Double precision
     &                  one, zero
c CGB Double precision  rho
      parameter         (one = 1.0D+0, zero = 0.0D+0)
c CGB parameter         (rho = 0.0D+0)
c
...
c
      ...
      dl  = -one / h2
      du  = -one / h2
c CGB  dl  = -one / h2 - 5.0D-1*rho / h
c CGB  du  = -one / h2 + 5.0D-1*rho / h
c 
c RBL y(1) =  dd*x(1) + du*x(2)
      y(1) =  (dd+h)*x(1) + du*x(2)
      do 10 j = 2,nx-1
c RBL    y(j) = dl*x(j-1) + dd*x(j) + du*x(j+1) 
         y(j) = dl*x(j-1) + (dd+j*h)*x(j) + du*x(j+1) 
 10   continue 
c RBL y(nx) =  dl*x(nx-1) + dd*x(nx) 
      y(nx) =  dl*x(nx-1) + (dd+nx*h)*x(nx) 
      return
      end
