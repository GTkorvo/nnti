...
int main(int argc, char *argv[]) 
{
  ...
  Teuchos::RefCountPtr<const Thyra::MPIVectorSpaceStd<double> > t_vs = 
    Teuchos::rcp( new Thyra::MPIVectorSpaceStd<double>(mpiComm,numme,numge) );
#else
  // make it a concrete Thyra Serial vectorspace
  Teuchos::RefCountPtr<const Thyra::SerialVectorSpaceStd<double> > t_vs = 
    Teuchos::rcp( new Thyra::SerialVectorSpaceStd<double>(numge) );
#endif
  Teuchos::RefCountPtr<MV> t_ivec = Thyra::createMembers(*t_vs,blockSize);
  ...
  Teuchos::RefCountPtr<Anasazi::BasicEigenproblem<double, MV, OP> > t_Problem =
    Teuchos::rcp( 
  ...
  if (!testFailed) {
    ...
    Teuchos::RefCountPtr<std::vector<double> > evals = t_Problem->GetEvals();
    Teuchos::RefCountPtr<MV> evecs = t_Problem->GetEvecs();
