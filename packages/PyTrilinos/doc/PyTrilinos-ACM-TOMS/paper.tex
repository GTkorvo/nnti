...
\title{PyTrilinos: Using High-Performance Parallel Distributed Memory
...
generation. \PyTrilinos\ vector objects are integrated with
...
interpreter.  The fundamental MPI calls are abstracted beneath an abstract
layer, that manages all inter-processor communications.  This makes serial and
parallel scripts using \PyTrilinos\ virtually identical.
\end{abstract}
...
CPU cycles (independent of the problem size) to parse each instruction.
...
compares PyTrilinos with MATLAB, and Section~\ref{sec:comparison_trilinos}
compares PyTrilinos with Trilinos.
...
other languages) interfaces for existing C and C++ code.  It also
...
\item {\bf Trilinos}, a set of numerical solver packages in active
  ...
  lines of code, and it can interface many third-party
...
\item {\bf ML} contains a set of multilevel preconditioners based on
  ...
  with Epetra and AztecOO. ML can use the
...
% This seems too detailed to me at this point. We are saying something later.
% MS
...
objects, we can define an MPI-based Epetra communicator 
(derived from the pure virtual class Epetra\_Comm class), on which all
...
serial runs, both MPI\_Init() and MPI\_Finalize() are automatically called
when the Epetra module is loaded and released, respectively.

\smallskip

The major disadvantage of this approach is that Python cannot be run
interactively if more than one processor is used. Besides,
%, and we experienced
...
\item Several utilities are available, for example Epetra.Time offers
  a clean and consistent access to timing routines.
...
created, an important problem is the solution of the linear system
\begin{equation}
  \label{eq:lin_sys}
  A X = B
\end{equation}
...
the factorization algorithm requires $\mathcal{O}(n^k)$ operations, with $k$
...
with a relatively modest number of processors. The forward and backward
...
% We never talked about Teuchos... I guess this is too detailed at this point..
...
time required by Trilinos (scale on the left), while the line reports the ratio between the time
required by PyTrilinos and the time required by Trilinos (scale on the right).}
...
\item {\bf Integration.} Python was designed to be a ``glue'' language
  ...
  different languages. 
  %C, C++ and FORTRAN code is called, but note that
  %Python programmers don't need to care.  
...
\item {\bf Software Quality.} Software quality is of vital importance
  ...
  challenging goal. Therefore, the
...
\item {\bf Performance Considerations.}  By using a Python wrapper, a
  ...
  %% Bill, this sentence is difficult to read and very technical. I skip it for
  %% the moment.
...
\item {\bf Management of C/C++ Arrays.} Although SWIG makes it easy to
  ...
  process still requires the programmer to define wrappers in the interface
...
also thank Michael Gee for suggestions on an earlier version of the manuscript.
